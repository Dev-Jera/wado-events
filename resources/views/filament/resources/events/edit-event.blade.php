...
document.addEventListener("DOMContentLoaded", function() {
    // Force all Filament grid containers to use full width
    function fixWidths() {
        // Target the main form container
        const formWrap = document.querySelector('.wado-form-wrap');
        if (!formWrap) return;
        
        // Find all grid containers and force them to block display with full width
        const grids = formWrap.querySelectorAll('.fi-grid, .fi-sc, .fi-sc-tabs, .fi-sc-tabs-tab, .fi-section, .fi-section-content');
        grids.forEach(grid => {
        ...
        // Force all grid columns to take full width
        ...
        // Force the two-column inner grids to also be block
        const innerGrids = formWrap.querySelectorAll('[style*="--cols-lg"]');
    ...
    const observer = new MutationObserver(fixWidths);
...
<style>
/* Force full width on page containers */
.fi-page-content,
.fi-main,
.fi-width-7xl {
...
.wado-edit-stat--revenue .wado-edit-stat__value { color: #c8102e; }
...
.wado-preview-ticket-price { color: #0d1b3e; font-weight: 700; }
.wado-preview-cta {
    display: flex;
    align-items: center;
    justify-content: space-between;
...
.wado-preview-price-tag { font-size: 0.8rem; font-weight: 800; color: #0d1b3e; }
.wado-preview-btn {
    background: #c8102e;
    color: #fff;
    border: none;
...
.wado-preview-link:hover { color: #0d1b3e; border-color: #0d1b3e; }

@media (max-width: 1100px) {
    .wado-edit-layout { flex-direction: column; }
    .wado-edit-right { width: 100%; position: static; }
    .wado-edit-left { width: 100%; }
}
</style>